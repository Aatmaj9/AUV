/**
 * @file   dvl-sensor.cpp
 * @author Pablo Gutiérrez (Updated for stability)
 * @date   24/11/2021
 */
...
Node("dvl_a50_node")
{
    ...
    timer_receive = this->create_wall_timer(std::chrono::milliseconds(40), std::bind(&DVL_A50::handle_receive, this));
    ...
    dvl_sub_config_command = this->create_subscription<dvl_msgs::msg::ConfigCommand>("dvl/config/command", qos, std::bind(&DVL_A50::command_subscriber, this, std::placeholders::_1));
    ...
    if(tcpSocket->Create() < 0) RCLCPP_ERROR(get_logger(), "Error creating socket");
    ...
    while(fault != 0) {
        ...
        if(error_code == 114 || error_code == 103) {
            RCLCPP_WARN(get_logger(), "Waiting for DVL... (Code: %d)", error_code);
            usleep(2000000);
            if(std::chrono::duration<double>(std::chrono::steady_clock::now() - start_time).count() > 80.0) {
                fault = -10; break;
            }
...
void DVL_A50::handle_receive() {
    ...
    while(true) {
        ...
        // Inner loop: Reassemble ONE line
        while(chars_read < max_chars) {
            ...
            if(r > 0) {
                ...
                // Socket is empty! Now we can finally exit the function.
                return; 
            }
        }

        // Validate the packet
        if(!found_newline || str.empty() || str[0] != '{') continue;

        try {
            json_data = json::parse(str);
            // Publish logic remains the same
            if (json_data.contains("altitude")) publish_vel_trans_report();
            else if (json_data.contains("pitch")) publish_dead_reckoning_report();
            else if (json_data.contains("response_to")) {
                if(json_data["response_to"] == "get_config") publish_config_status();
                else publish_command_response();
            }
        } catch(...) { 
            // If one packet is corrupted, skip it and try the next one in the buffer
        }
    }
}
...
void DVL_A50::publish_vel_trans_report() {
    ...
    dvl.velocity.x = json_data["vx"];
    dvl.velocity.y = json_data["vy"];
    dvl.velocity.z = json_data["vz"];
    dvl.altitude = json_data["altitude"];
...
void DVL_A50::publish_dead_reckoning_report() {
    ...
    msg.position.x = json_data["x"];
    msg.position.y = json_data["y"];
    msg.position.z = json_data["z"];
    msg.roll = json_data["roll"];
    msg.pitch = json_data["pitch"];
    msg.yaw = json_data["yaw"];
...
void DVL_A50::command_subscriber(const dvl_msgs::msg::ConfigCommand::SharedPtr msg) {
    set_json_parameter(msg->parameter_name, msg->parameter_value);
}
