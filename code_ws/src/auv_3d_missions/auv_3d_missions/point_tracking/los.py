...
class PointTrackingMission3dLos(Node):
    """3D point tracking using simple LOS guidance (no integral correction)."""

    def __init__(self) -> None:
        super().__init__("point_tracking_mission_3d_los")
    ...
    def _on_timer(self) -> None:
        ...
        if self._done:
            ...
            self._publish_status("point_tracking_3d_los: completed")
            return
        ...
        # Use signed horizontal cross-track so yaw correction distinguishes port/starboard error.
        ...
        psi_d = chi - math.atan2(e_xy_signed, max(1e-3, look_xy))
        ...
        self._publish_status(
            f"point_tracking_3d_los: leg={i0 + 1}->{i1 + 1}/{len(self._waypoints)} "
...
def main(args=None) -> None:
    ...
    node = PointTrackingMission3dLos()
    try:
        rclpy.spin(node)
    finally:
        node.destroy_node()
