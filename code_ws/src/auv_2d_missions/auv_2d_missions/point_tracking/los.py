...
class PointTrackingMission2dLos(Node):
    """2D point tracking with simple LOS guidance."""

    def __init__(self) -> None:
        super().__init__("point_tracking_mission_2d_los")
    ...
    def _on_timer(self) -> None:
        ...
        if self._done:
            ...
            self._publish_status("point_tracking_2d_los: completed")
            return
        ...
        psi_d = chi_path - math.atan2(e_y, max(1e-3, lookahead))
        self._publish_reference(max(u_min, min(u_max, u_cruise)), 0.0, psi_d)
        self._publish_status(f"point_tracking_2d_los: leg={i0 + 1}->{i1 + 1}/{len(self._waypoints)} cross={e_y:.2f}m")


def main(args=None) -> None:
    rclpy.init(args=args)
    node = PointTrackingMission2dLos()
    try:
        rclpy.spin(node)
    finally:
        node.destroy_node()
